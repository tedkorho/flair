...
def Model_flare(Fres, t, model):
    ...
    popt, pcov = curve_fit(f, t, Fres)
    
    return (popt, [f(t, *popt) for time in t])
...
def E_flare_model(iamps, itimes, T_flare, LpS, LpF, R_star):
    ...
    energy = simps(L_f(Fmodel[1]), times)
...
def main(): 
   ...
   for i in range(len(flare_energies)):
       print("{:.6f} {:.6e}".format(flare_times[i], flare_energies[i], flare_impulses[i]))
